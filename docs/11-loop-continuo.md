...
**Experimento rápido: blend (média) dos pesos v12 + checkpoint explore gen26** — testando se o meio-termo entre dois pontos com perfis de força diferentes capturaria o melhor dos dois. Validado a n=60: **83.4% ponderado — também empate técnico**, não uma combinação melhor. Confirma que a banda 83-84% parece ser o teto prático real desta arquitetura com o sinal de fitness atual, não um artefato de qual ponto específico dentro dela se escolhe.

**Decisão de segurança (16/08, prazo se aproximando)**: com três resultados seguidos empatados dentro do ruído (nenhum claramente melhor que a v12) e ~3h restantes até o prazo, submetida `v13` — reenvio da v12 sem nenhuma mudança de código, só para garantir que o dia tenha pelo menos uma submissão confirmada antes do prazo (ver critério de desempate de `docs/10`). 4 de 5 submissões restantes hoje depois desta. Busca continua em background para as próximas iterações, se houver ganho real a validar.
