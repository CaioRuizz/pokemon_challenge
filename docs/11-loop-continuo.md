...
**Correção estrutural na função de fitness** (`scripts/evolve_policy.py`): adicionada uma penalidade de piso (`_FLOOR_WINRATE=0.40`, `_FLOOR_PENALTY_SCALE=0.3`) aplicada de forma **absoluta**, não escalada pelo peso de uso do arquétipo — qualquer arquétipo abaixo de 40% de winrate agora custa fitness de verdade, independente de quão pouco ele pesa na média. Isso não muda o objetivo principal (ainda é maximizar a média ponderada — é isso que importa pro score real), só remove a possibilidade de "pagar" essa média às custas de arquétipos minoritários caindo abaixo de um piso razoável. Relançado (`data/ml/evolved_weights_v2_floor.json`, novo arquivo — os valores de fitness antigos e novos não são comparáveis, a função mudou; pesos mantidos como warm-start, fitness resetado). Em andamento.
